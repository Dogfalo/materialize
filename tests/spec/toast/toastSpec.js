describe('Toasts:', function() {
  let toastOutDuration = 375;
  let toastInDuration = 300;
  ...
  describe('Toast javascript functions', function() {
    ...
    it('should display and remove a toast', function(done) {
      M.toast({ html: 'Test toast', displayLength: toastInDuration, classes: 'customDuration' });

      setTimeout(function() {
        let toast = $('.toast.customDuration');
        expect(toast.length).toBe(1);
        expect(toast).toBeVisible();
        expect(toast.text()).toBe('Test toast');
        setTimeout(function() {
          toast = $('.toast.customDuration');
          expect(toast).toBeVisible();
    ...
    it('Opens a toast with HTML content', function() {
      ...
      M.toast({ html: $toastContent, displayLength: 400, classes: 'customContent'});
      let toast = $('.toast.customContent');
    ...
    it('Apply two custom class to a toast', function() {
      ...
      let toast = $('.toast.round.flat');
      expect(toast.length).toBe(
        1,
